# შექმენი ცარიელი dictionary thisdict, და შეავსე რამდენიმე nested dictით (კოდმა გააკეთოს)
...
for x in range(people):#this is for starting the loop
    ...
    thisdict[dictname]["name"] = name#created key named name in the nested dictionary
    y = y + 1#count increaser
print(thisdict)#printing the main dictionary with it's nested dictionaries
a = input("do you want to edit or remove (to exit type exit): ")#defining the next steps 

#This part is for editing the Dictionary

if a == "edit":#if edit is chosen
    ...
    if dict_edit == "name":#if name is chosen to be edited
        ...
        thisdict[dict_nameedit]["name"] = dict_edit_name#accessing the key of the selected nested dictionary
